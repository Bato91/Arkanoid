    ...
    def draw_wall(self):
        for row in self.blocks:
            for block in row:
                ...
                pygame.draw.rect(screen, 255, block[0], 2)
...
while run:
    ...
    if not live_ball:
        if game_over == 0:
            draw_text('PRESIONE CUALQUIER TECLA...', font, text_col, 100, screen_height // 2 + 100)
        elif game_over == 1:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            run = False
        if event.type == pygame.KEYUP and live_ball == False:
            live_ball = True
